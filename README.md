...
- The OpenAI API key is stored as plain text in `chrome.storage.local` on your machine. Avoid installing this extension on shared machines unless you trust all users. You can clear the saved key at any time:
  - In the OntoPrompt popup, delete the key and click **Save**.
  - Or open Chrome DevTools on `https://yasgui.triply.cc/#`, run:
    ```
